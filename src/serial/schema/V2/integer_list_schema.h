...
#ifndef DINGO_SERIAL_INTEGER_LIST_SCHEMA_V2_H_
#define DINGO_SERIAL_INTEGER_LIST_SCHEMA_V2_H_
...
class DingoSchema<std::vector<int32_t>> : public BaseSchema {
 public:
  ...
  BaseSchemaPtr Clone() override {
    return std::make_shared<DingoSchema<std::vector<int32_t>>>();
  }
 ...
 private:
  ...
  void DecodeIntList(Buf& buf, std::vector<int32_t>& data, int offset);
};
